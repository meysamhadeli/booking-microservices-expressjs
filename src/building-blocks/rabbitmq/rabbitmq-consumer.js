...
let Consumer = class Consumer {
  async addConsumer(consumerOptions) {
    const rabbitMQConnection = tsyringe_1.container.resolve(
            ...
            (message) => {
              if (message !== null) {
                ...
                consumerOptions.handler(
                  q.queue,
                  (0, serialization_1.deserializeObject)(messageContent)
                );
                logger_1.Logger.info(
...
exports.Consumer = Consumer = __decorate([(0, tsyringe_1.injectable)()], Consumer);
