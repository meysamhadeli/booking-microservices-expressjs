...
let CreateBookingController = class CreateBookingController extends tsoa_1.Controller {
    createBooking(request) {
        return __awaiter(this, void 0, void 0, function* () {
            const result = yield mediatr_js_1.mediatrJs.send(new CreateBooking({
...
let CreateBookingHandler = class CreateBookingHandler {
    ...
    handle(request) {
        var _a, _b;
        return __awaiter(this, void 0, void 0, function* () {
            yield createBookingValidations.validateAsync(request);
            const flightDto = yield this.flightClientService.getFlightById(request.flightId);
            const passengerDto = yield this.passengerClientService.getPassengerById(request.passengerId);
            const avalibaleSeats = yield this.flightClientService.getAvalibaleSeats(request.flightId);
            if (avalibaleSeats.length == 0) {
                throw new notFoundException_1.default('No seat available!');
            }
            yield this.flightClientService.reserveSeat({
            ...
            const bookingEntity = yield this.bookingRepository.createBooking(new booking_1.Booking({
