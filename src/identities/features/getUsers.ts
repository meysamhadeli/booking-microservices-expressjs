...
export class GetUsersController extends Controller {
  ...
  public async getUsers(
    ...
    const result = await mediatrJs.send<PagedResult<UserDto[]>>(new GetUsers({
      page: page, pageSize: pageSize, searchTerm: searchTerm, order: order, orderBy: orderBy
    }));
...
mediatrJs.registerHandler(GetUsers, getUsersHandler);
